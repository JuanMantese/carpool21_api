...
export class AuthService {
  ...
  async login(loginDTO: LoginAuthDTO) {
    ...
    // Encriptar y guardar el refreshToken en la base de datos
    const hashedRefreshToken = await hash(refreshToken, 10);
    await this.usersRepository.update(userFound.idUser, { refreshToken: hashedRefreshToken });
  ...
  async refreshToken(refreshToken: string) {
    try {
      // Decodificar el payload del refreshToken sin verificar aún
      const payload = this.jwtService.verify(refreshToken, { secret: jwtConstants.secret });

      // Buscar al usuario y cargar sus roles mediante la relación userXrol
      const user = await this.usersRepository.findOne({
        where: { idUser: payload.idUser },
        relations: ['userRoles', 'userRoles.role'], // Cargar roles relacionados
      });
      ...
      // Validar el refreshToken comparando con el almacenado (encriptado)
      const isRefreshTokenValid = await compare(refreshToken, user.refreshToken);
      if (!isRefreshTokenValid) {
        throw new HttpException('Refresh token no válido', HttpStatus.UNAUTHORIZED);
      }

      // Obtener los roles del usuario
      const roles = user.userRoles
        .filter((userRole) => userRole.status) // Solo roles activos
        .map((userRole) => userRole.role.idRole); // Extraer los IDs de los roles

      // Generar un nuevo payload con los datos del usuario
      const newPayload = { idUser: user.idUser, name: user.name, roles };

      // Generar un nuevo token principal
      const newToken = this.jwtService.sign(newPayload);

      // Generar un nuevo refreshToken y actualizar en la base de datos
      const newRefreshToken = this.jwtService.sign(newPayload, { expiresIn: '7d' });
      const hashedNewRefreshToken = await hash(newRefreshToken, 10);
      await this.usersRepository.update(user.idUser, { refreshToken: hashedNewRefreshToken });

      return { token: newToken, refreshToken: newRefreshToken };
    } catch (err) {
      if (err.name === 'TokenExpiredError') {
        throw new HttpException('El refresh token ha expirado', HttpStatus.UNAUTHORIZED);
      }
